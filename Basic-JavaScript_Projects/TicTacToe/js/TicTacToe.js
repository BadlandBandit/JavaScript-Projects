...
let activePlayer = 'X';
//Array to store moves - used to determine win conditions
let selectedSquares = [];
...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        //Determines the active player and places the corresponding image
        if (activePlayer === 'X') {
        ...
        //Calls the function to check for win conditions
        checkWinConditions();
    ...
    //If square is already selected, return false
    return false;
}
...
//The drawline function is called to draw a line on the screen if a win condition is met
function checkWinConditions() {
    ...
    //Check for tie - if no win conditions are met and all squares are selected
...
// Dummy audio function if not defined elsewhere
function audio(src) {
    let audio = new Audio(src);
    audio.play();
}

//Draws a line on the screen to indicate a win
...
//Resets the game board and clears all selections
function resetGame() {
    for (let i = 0; i < 9; i++) {
